...
namespace Utility
{
    ...
    {
        private static readonly int ImageUrlsLimited = 32;
        ...
        public async static Task UploadAsync(string trainingKey, string projectId, string foodName, List<string> imageUrlList)
        {
            ...
            imageUrlList.Chunk(ImageUrlsLimited).ToList().ForEach(async x =>
            {
                var result = await client.CreateImagesFromUrlsAsync(guid, new ImageUrlCreateBatch
                {
